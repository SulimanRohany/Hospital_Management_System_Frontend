...
const ALLOWED_ROOTS = new Set(["users", "audit-logs", "departments"]);
...
function backendPath(request, segments) {
  ...
  return `/api/v1/${path}/${new URL(request.url).search}`;
}
...
async function forward(request, context, method) {
  ...
  if (!target) {
    return NextResponse.json(
      { message: "Unsupported account endpoint." },
      { status: 404 }
    );
  }
  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { message: errorMessage(payload), errors: payload },
